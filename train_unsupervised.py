...
from utils.warp_loss_function import *
EPSILON = 1e-8
...
def train(model, optimizer, train_loader, selection, temp, logger, ep, iter, p=0.5, weighted=False, rank=0, model_type='cvpr'):
    ...
    for i, (imgs, imus, gts, rot, weight, intric) in enumerate(train_loader):
        ...
        Input = imgs[:,args.seq_len//2]

        if model_type != 'deepvio' and model_type != "svio_vo" and model_type != "flowformer_vo" and model_type != "gmflow_vo" and model_type != "flowformer_vio" and model_type != "flowformer_vio_lstm" and model_type != "flowformer_extractor_vo" and model_type != "flowformer_extractor_nocorr_vo" and model_type != "flowformer_vo_part_corr" and model_type!="flowformer_vo_unsupervised":
            poses, pose_backward = [], []
        ...
        elif model_type == "flowformer_vo_unsupervised":
            poses=[]
            two_imgs_forward = torch.cat((imgs[:, :-1], imgs[:, 1:]), dim=2)
            for j in range(two_imgs_forward.size(1)):
                img0 = two_imgs_forward[:, j, :3]
                img1 = two_imgs_forward[:, j, 3:]
                pose, depth2, depth = model(img0,img1,Input)
                poses.append(pose.squeeze(1))
        ...
        if not weighted:# 6DoF pose前三位是rotation，后三位是translation
        ...
        # mat = pose_vec2mat4(poses.squeeze(1),device=device,rotation_mode="euler")
        pose = out2pose(poses)
        # 增加一致性损失，cvpr的额外损失
        ...
        res_imgs = torch.cat([imgs[:,:args.seq_len//2],imgs[:,args.seq_len//2+1:]], dim=1)
        loss_photo, loss_3d = photometric_reconstruction_loss(imgs[:,args.seq_len//2], ref_imgs=res_imgs, intrinsics=intric,
                                                     depth=depth2[:scales], pose=pose, rotation_mode='euler', padding_mode='zeros', ref_depth=[None]*scales)
        loss_vo1 = torch.zeros(1).to(device)
        loss_vo2 = torch.zeros(1).to(device)
        ...
        if i % args.print_frequency == 0: 
            ...
            message = f'Epoch: {ep}, iters: {i}/{data_len}, iter: {iter}/{data_len}, loss_photo: {loss_photo.item():.6f}, loss_3d: {loss_3d}, loss_vo1: {loss_vo1.item():.6f}, loss_vo2: {loss_vo2.item():.6f}, loss_smooth: {loss_smooth.item():.6f}, angle_loss: {angle_loss.item():.6f}, translation_loss: {translation_loss.item():.6f}, penalty: {penalty.item():.6f}, loss: {loss.item():.6f}'
            print(message)
            if args.experiment_name != 'debug':
                ...
                wandb.log({"Epoch": ep, "iters": i, "iter": iter, "loss_photo": loss_photo.item(),"penalty": penalty, "loss_3d": loss_3d, "loss_vo1": loss_vo1.item(), "loss_vo2": loss_vo2.item(), "loss_smooth": loss_smooth.item(), "angle loss": angle_loss.item(), "translation loss": translation_loss.item(), "loss": loss.item()})
            logger.info(message)
...
def pose_vec2mat4(vec, rotation_mode='euler'):
    ...
    # print("rotation",rot_mat.shape)
    # print("translation",translation.shape)
    transform_mat = torch.zeros((translation.size(0),3,4)).to(translation.device)
    # transform_mat = torch.cat([rot_mat, translation], dim=2)  # [B, 3, 4]
    transform_mat[:,:3,:3] = rot_mat
    transform_mat[:,:3,3] = translation.squeeze(-1)
    add = torch.FloatTensor([0, 0, 0, 1]).view(1, 4).unsqueeze(0).repeat(transform_mat.size(0), 1, 1).to(translation.device)
...
def main():
    ...
    elif args.model_type == 'flowformer_vo' or "flowformer_vo_unsupervised":
