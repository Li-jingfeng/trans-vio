...
from .inverse_warp import inverse_warp_3d
...
def photometric_reconstruction_loss(tgt_img, ref_imgs, intrinsics,
    ...
    def one_scale(depth, explainability_mask = None, ref_depth=None):
        # assert(pose.size(1) == ref_imgs.size(0))

        reconstruction_loss, loss_3d = 0, 0
        b, _, h, w = depth.size()
        downscale = tgt_img.size(2)/h
        # 这句自己加的
        tmp_ref_imgs = ref_imgs.permute(1, 0, 2, 3, 4)
        tgt_img_scaled = F.interpolate(tgt_img, (h, w), mode='area')
        ref_imgs_scaled = [F.interpolate(ref_img, (h, w), mode='area') for ref_img in tmp_ref_imgs]
