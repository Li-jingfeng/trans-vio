...
class MemoryEncoder(nn.Module):
    ...
    def forward(self, img1, img2, data, context=None):
        ...
        return feat_s, feat_t, B, C, H, W

# flowformer transformer feature extractor
class FlowFormer_Extractor_nocorr_VO(nn.Module):
    def __init__(self, cfg, regression_mode=2):
        super(FlowFormer_Extractor_nocorr_VO, self).__init__()
        self.cfg = cfg
        ...
        if self.regression_mode == 2:
            self.out_dim = cfg.image_size[0]*cfg.image_size[1]/64*8
            self.regressor_s_1 = nn.Sequential(
                nn.Linear(256, 128),
                nn.LeakyReLU(0.1, inplace=True),
                nn.Linear(128, 8),
                nn.LeakyReLU(0.1, inplace=True),
            ...
            self.regressor_s_2 = nn.Sequential(
            ...
            # self.regressor_t_1 = nn.Sequential(
            #     nn.Linear(256, 128),
            #     nn.LeakyReLU(0.1, inplace=True),
            #     nn.Linear(128, 8),
            #     nn.LeakyReLU(0.1, inplace=True),
            ...
            #     # nn.Linear(128, 8),
            #     )
            # self.regressor_t_2 = nn.Sequential(
            #     nn.Linear(int(8*cfg.image_size[0]*cfg.image_size[1]/64), 2048),
            #     nn.LeakyReLU(0.1, inplace=True),
            #     nn.Linear(2048, 512),
            #     nn.LeakyReLU(0.1, inplace=True),
    ...
    def forward(self, image1, image2, output=None, flow_init=None):
        ...
        data = {}
        feat_s, feat_t, B, C, H, W = self.memory_encoder(image1, image2, data, None)
        feat_s = feat_s.permute(0,2,3,1)
        ...
        # 2.先reshape 128*8->8，再reshapeH*W*8
            ...
            x_s = self.regressor_s_1(feat_s)
            x_s = x_s.reshape(B,-1)
            x_s = self.regressor_s_2(x_s)

            x_t = self.regressor_s_1(feat_t)
            x_t = x_t.reshape(B,-1)
            x_t = self.regressor_s_2(x_t)
            # x_t = self.regressor_t_1(feat_t)
            # x_t = x_t.reshape(B,-1)
            # x_t = self.regressor_t_2(x_t)
