    ...
    def forward(self, img, imu, is_first=True, hc=None, temp=5, selection='gumbel-softmax', p=0.5):
        ...
        for i in range(seq_len):
            if i == 0 and is_first:
                ...
                pose, hc, teacher_kd_feature = self.Pose_net(fv[:, i:i+1, :], None, fi[:, i:i+1, :], None, hc)
            else:
                if selection == 'gumbel-softmax':
                    ...
                    pose, hc, teacher_kd_feature = self.Pose_net(fv[:, i:i+1, :], fv_alter[:, i:i+1, :], fi[:, i:i+1, :], decision, hc)
        ...
        return poses, hc, teacher_kd_feature
