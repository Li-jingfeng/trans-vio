...
from flowformer.utils.utils import coords_grid, bilinear_sampler, upflow8
from flowformer.attention import BroadMultiHeadAttention, MultiHeadAttention, LinearPositionEmbeddingSine, ExpPositionEmbeddingSine
from flowformer.encoders import twins_svt_large
from typing import Optional, Tuple
from flowformer.twins import Size_, PosConv
from flowformer.cnn import TwinsSelfAttentionLayer, TwinsCrossAttentionLayer, BasicEncoder
from flowformer.mlpmixer import MLPMixerLayer
from flowformer.convnext import ConvNextLayer
import time

from timm.models.layers import Mlp, DropPath, trunc_normal_ #, activations, to_2tuflowple
...
from flowformer.twins import Block, CrossBlock
...
class CostPerceiverEncoder(nn.Module):
    ...
    def forward(self, cost_volume, data, context=None):
        B, seq_len, heads, H1, W1, H2, W2 = cost_volume.shape
        cost_volume = cost_volume.view(B*seq_len,heads,H1,W1,H2,W2)
        cost_maps = cost_volume.permute(0, 2, 3, 1, 4, 5).contiguous().view(B*seq_len*H1*W1, self.cfg.cost_heads_num, H2, W2)
        data['cost_maps'] = cost_maps

        if self.cost_scale_aug is not None:# None
            scale_factor = torch.FloatTensor(B*H1*W1*seq_len, self.cfg.cost_heads_num, H2, W2).uniform_(self.cost_scale_aug[0], self.cost_scale_aug[1]).cuda()
            cost_maps = cost_maps * scale_factor
        
        x, size = self.patch_embed(cost_maps)   # B*H1*W1*seq_len, size[0]*size[1], C
        data['H3W3'] = size
        H3, W3 = size

        x = self.input_layer(self.latent_tokens, x)# self.latent_tokens的作用

        # short_cut = x # [B*H1*W1,8,dim]
...
class MemoryEncoder(nn.Module):
    ...
    def corr(self, fmap1, fmap2): # fmap1 = source && fmap2 = target
        ...
        batch, seq_len, dim, ht, wd = fmap1.shape
        fmap1 = rearrange(fmap1, 'b s (heads d) h w -> (b s) heads (h w) d', heads=self.cfg.cost_heads_num)# heads=1
        fmap2 = rearrange(fmap2, 'b s (heads d) h w -> (b s) heads (h w) d', heads=self.cfg.cost_heads_num)
        corr = einsum('bhid, bhjd -> bhij', fmap1, fmap2)
        corr = corr.permute(0, 2, 1, 3).view(batch*seq_len*ht*wd, self.cfg.cost_heads_num, ht, wd)
        #corr = self.norm(self.relu(corr))
        corr = corr.view(batch*seq_len, ht*wd, self.cfg.cost_heads_num, ht*wd).permute(0, 2, 1, 3)
        corr = corr.view(batch, seq_len, self.cfg.cost_heads_num, ht, wd, ht, wd)

        return corr # [b,seq_len,head,h,w,h,w]->head==1

    def forward(self, image, data, context=None):# img1,img2
        ...
        B,seq_len,channel,oh,ow = image.shape[0],image.shape[1],image.shape[2],image.shape[3],image.shape[4]
        imgs = image.view(B*seq_len,channel,oh,ow)
        feats = self.feat_encoder(imgs) # 与context feature 提取方式一样
        feats = self.channel_convertor(feats)
        # B = feats.shape[0] // 2
        feats = feats.view(B,seq_len,feats.shape[1],feats.shape[2],feats.shape[3])

        feat_s = feats[:,:-1]
        feat_t = feats[:,1:]

        B, seq_len, C, H, W = feats.shape
        size = (H, W)

        # if self.cfg.feat_cross_attn: # False in default
        #     feat_s = feat_s.flatten(2).transpose(1, 2)
        #     feat_t = feat_t.flatten(2).transpose(1, 2)a
        ...
        x = self.cost_perceiver_encoder(cost_volume, data, context)# [H/8*W/8,8,128] context=[b,seq_len-1,dim,h/8,w/8] cost_volume=[b,seq_len-1,1,h,w,h,w]
