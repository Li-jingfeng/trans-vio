...
class FlowFormer_VO_Unsupervised(nn.Module):
    def __init__(self, cfg, args, regression_mode=2):
        super(FlowFormer_VO_Unsupervised, self).__init__()
        self.cfg = cfg
        ...
        # DepthNet
        self.disp_net = models.DepthDecoder(alpha=10, beta=0.01)
        ...
        if self.regression_mode == 2:
            ...
            self.regressor_2 = nn.Sequential(
                nn.Linear(int(8*args.img_h*args.img_w/64), 2048),
                nn.LeakyReLU(0.1, inplace=True),
                nn.Linear(2048, 512),
                nn.LeakyReLU(0.1, inplace=True),
                nn.Linear(512, 128),
    ...
    def forward(self, image1, image2, input, output=None, flow_init=None):
        # Following https://github.com/princeton-vl/RAFT/
        image1 = 2 * (image1 / 255.0) - 1.0
        image2 = 2 * (image2 / 255.0) - 1.0
        # depth
        disp = self.disp_net(input)
        ...
        return pose.unsqueeze(1), depth2, depth
        # return cost_memory
